...
func DecodeInstruction(decoder *goloxi.Decoder) (IInstruction, error) {
	...
	case 7:
		return DecodeInstructionStatTrigger(_instruction, decoder)
	case 65535:
...
type InstructionStatTrigger struct {
	*Instruction
	Flags      StatTriggerFlags
	Thresholds []IOxs
}

type IInstructionStatTrigger interface {
	IInstruction
	GetFlags() StatTriggerFlags
	GetThresholds() []IOxs
}
...
func (self *InstructionStatTrigger) Serialize(encoder *goloxi.Encoder) error {
	...
	encoder.PutUint32(uint32(self.Flags))
	for _, obj := range self.Thresholds {
		if err := obj.Serialize(encoder); err != nil {
			return err
		}
	}
...
func DecodeInstructionStatTrigger(parent *Instruction, decoder *goloxi.Decoder) (*InstructionStatTrigger, error) {
	_instructionstattrigger := &InstructionStatTrigger{Instruction: parent}
	if decoder.Length() < 4 {
		return nil, fmt.Errorf("InstructionStatTrigger packet too short: %d < 4", decoder.Length())
	}
	_instructionstattrigger.Flags = StatTriggerFlags(decoder.ReadUint32())
	...
	for decoder.Length() >= 4 {
		...
		if item != nil {
			_instructionstattrigger.Thresholds = append(_instructionstattrigger.Thresholds, item)
		}
	}
	return _instructionstattrigger, nil
}

func NewInstructionStatTrigger() *InstructionStatTrigger {
	obj := &InstructionStatTrigger{
		Instruction: NewInstruction(7),
	}
	return obj
}
